require 'rails_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to test the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator. If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails. There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.

RSpec.describe "/habits", type: :request do
  # This should return the minimal set of attributes required to create a valid
  # Habit. As you add validations to Habit, be sure to
  # adjust the attributes here as well.
  let(:valid_attributes) {
    { name: 'sample', streak: 5 }
  }

  let(:invalid_attributes) {
    { name: '', streak: 'sample' }
  }

  # This should return the minimal set of values that should be in the headers
  # in order to pass any filters (e.g. authentication) defined in
  # HabitsController, or in your router and rack
  # middleware. Be sure to keep this updated too.
  let(:valid_headers) {
    {}
  }

  before(:each) do
    unless RSpec.current_example.metadata[:skip_before_each]
      @habit = Habit.create! valid_attributes
    end
  end

  describe "GET /index" do
    it "renders a successful response" do
      get api_v1_habits_url, headers: valid_headers, as: :json
      expect(response).to be_successful
    end
  end

  describe "GET /show" do
    it "renders a successful response" do
      get api_v1_habit_url(@habit), as: :json
      expect(response).to be_successful
    end
  end

  describe "POST /create", :skip_before_each do
    context "with valid parameters" do
      it "creates a new Habit" do
        expect {
          post api_v1_habits_url,
               params: { habit: valid_attributes }, headers: valid_headers, as: :json
        }.to change(Habit, :count).by(1)
      end

      it "renders a JSON response with the new habit" do
        post api_v1_habits_url,
             params: { habit: valid_attributes }, headers: valid_headers, as: :json
        expect(response).to have_http_status(:created)
        expect(response.content_type).to match(a_string_including("application/json"))
      end
    end

    context "with invalid parameters" do
      it "does not create a new Habit" do
        expect {
          post api_v1_habits_url,
               params: { habit: invalid_attributes }, as: :json
        }.to change(Habit, :count).by(0)
      end

      it "renders a JSON response with errors for the new habit" do
        post api_v1_habits_url,
             params: { habit: invalid_attributes }, headers: valid_headers, as: :json
        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.content_type).to match(a_string_including("application/json"))
      end
    end
  end

  describe "PATCH /update" do
    context "with valid parameters" do
      let(:new_attributes) {
        { name: 'Updated', streak: 10 }
      }

      it "updates the requested habit" do
        patch api_v1_habit_url(@habit),
              params: { habit: new_attributes }, headers: valid_headers, as: :json
        @habit.reload
        expect(@habit.name).to eq(new_attributes[:name])
        expect(@habit.streak).to eq(new_attributes[:streak])
      end

      it "renders a JSON response with the habit" do
        patch api_v1_habit_url(@habit),
              params: { habit: new_attributes }, headers: valid_headers, as: :json
        expect(response).to have_http_status(:ok)
        expect(response.content_type).to match(a_string_including("application/json"))
      end
    end

    context "with invalid parameters" do
      it "renders a JSON response with errors for the habit" do
        patch api_v1_habit_url(@habit),
              params: { habit: invalid_attributes }, headers: valid_headers, as: :json
        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.content_type).to match(a_string_including("application/json"))
      end
    end
  end

  describe "DELETE /destroy" do
    it "destroys the requested habit" do
      expect {
        delete api_v1_habit_url(@habit), headers: valid_headers, as: :json
      }.to change(Habit, :count).by(-1)
    end
  end
end
